...
Le cas d'usage `GetAnalyticsDataUseCase` est l'orchestrateur de cette agrégation. Il récupère, pour un utilisateur donné, l'ensemble des projets associés, leurs tâches et leur historique SGR, puis calcule sept séries de données distinctes regroupées en deux blocs fonctionnels.

Le premier bloc, intitulé "Tasks Overview", centralise quatre indicateurs de flux agrégés. La répartition par statut (To Do / In Progress / Done) et par priorité (Low / Medium / High) donnent une photographie instantanée de la charge globale. La courbe de vélocité présente le nombre de tâches terminées par semaine ISO sur les douze dernières semaines — une fenêtre identique à celle utilisée par R_Throughput dans l'algorithme SGR, ce qui permet à l'utilisateur de corréler visuellement la tendance du débit avec l'évolution du score. Le taux de complétion par projet, exprimé en pourcentage de tâches "Done" sur le total, permet d'identifier immédiatement les projets à l'arrêt.

Le second bloc, intitulé "Risk Score (SGR)", propose trois vues complémentaires centrées sur la donnée de risque. Le composant `SGRMultiChart` trace, sur un seul graphique à courbes multiples, l'évolution temporelle du SGR de chaque projet — visualisation impossible depuis le `SGRWidget`, qui n'affiche que la courbe du projet courant. La distribution des niveaux de risque (Faible / Modéré / Élevé / Critique) rend compte de la fréquence historique à laquelle chaque seuil a été atteint, tous projets confondus. Le tableau des derniers scores offre une lecture rapide du risque courant de chaque projet, classés du plus critique au plus faible.

**Figure 3.19 — Capture d'écran : tableau de bord Analytics — Bloc 1 (vue tâches)**

[📸 CAPTURE D'ÉCRAN : quatre cartes Recharts disposées en grille 2×2 — en haut à gauche un graphique circulaire de répartition par statut (To Do / In Progress / Done), en haut à droite un graphique à barres par priorité (Low / Medium / High), en bas à gauche une courbe en aire de vélocité sur 12 semaines (axe des abscisses : labels W01–W12, axe des ordonnées : nombre de tâches), en bas à droite un graphique à barres horizontales de taux de complétion par projet avec pourcentages.]

**Pourquoi cette capture.** Elle prouve que les métriques de flux utilisées par l'algorithme SGR — débit hebdomadaire, répartition des statuts — sont également exposées à l'utilisateur sous une forme lisible et agrégée. L'utilisateur ne consulte pas le SGR comme une boîte noire : il peut remonter aux données brutes qui l'alimentent. **Lien avec le SGR :** la courbe de vélocité du Bloc 1 partage la même fenêtre de 12 semaines que R_Throughput, permettant à l'équipe de comprendre intuitivement pourquoi le score de débit a évolué — condition nécessaire pour qu'une alerte SGR déclenche une action corrective et non une simple curiosité.

**Figure 3.20 — Capture d'écran : tableau de bord Analytics — Bloc 2 (SGR croisé)**

[📸 CAPTURE D'ÉCRAN : en haut, `SGRMultiChart` — courbes en aire superposées pour chaque projet, chacune d'une couleur distincte, axe des abscisses avec dates, axe des ordonnées de 0 à 100, trois lignes pointillées aux seuils 30/60/80 ; en bas à gauche, `SGRLevelDistribution` — graphique circulaire avec les quatre niveaux colorés (vert, jaune, orange, rouge) et leurs pourcentages ; en bas à droite, `SGRScoreTable` — tableau à trois colonnes (Projet, Score, Niveau) avec badge coloré sur la colonne Niveau.]

**Pourquoi cette capture.** Le `SGRWidget` prouve que le score est calculé et affiché ; cette capture prouve que la dimension *comparative* existe. Un utilisateur qui supervise plusieurs équipes peut, d'un seul regard, identifier quel projet a le score le plus dégradé et quelle tendance temporelle prévaut. **Lien avec le SGR :** la vision transversale du Bloc 2 est la réponse opérationnelle à l'argument théorique central du mémoire — la proactivité n'a de valeur que si elle est disponible au bon niveau de granularité, c'est-à-dire aussi bien au niveau du projet (SGRWidget) qu'au niveau du portefeuille (SGR croisé Analytics).

Sur le plan architectural, les sept composants Recharts sont chargés dynamiquement (`next/dynamic` avec `ssr: false`) depuis un composant client enveloppe `AnalyticsClient`. Ce choix est imposé par une contrainte de Next.js 16 : les composants Recharts font appel à des interfaces du navigateur (`window`, `ResizeObserver`) incompatibles avec le rendu serveur. L'agrégation des données est réalisée côté serveur par `GetAnalyticsDataUseCase`, appelé depuis l'Action serveur `getAnalyticsData()`, avant transmission au composant client. Cette séparation respecte le principe de responsabilité unique : le serveur agrège, le client visualise.
